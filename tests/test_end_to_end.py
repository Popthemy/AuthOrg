...
class TestRegisterEndpoint:
    ...
    def test_login_user_success_returns_200(self,api_client):
        ...
        url_login = reverse_lazy('login_user')  # Replace 'login_user' with your actual login endpoint name
        ...
        assert login_response.data['data']['user']['email'] == login_data['email'], f"Expected email {login_data['email']} but got {login_response.data['data']['user']['email']}"
    ...
    def test_missing_required_fields_returns_422(self, api_client):
        ...
        for data, missing_field in missing_field_tests:
            response = api_client.post(reverse_lazy('register_user'), data, format='json')
            assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY, f"Expected status code 422 but got {response.status_code}"
            assert missing_field in response.data['errors'], f"Expected error for missing field {missing_field} but got {response.data['errors']}"
            assert len(response.data['errors'][missing_field]) == 1, f"Expected exactly one error message for field {missing_field} but got {len(response.data['errors'][missing_field])}"
    ...
    def test_duplicate_email_registration_returns_422(self, api_client):
        ...
        api_client.post(reverse_lazy('register_user'), register_data, format='json')
        ...
        response = api_client.post(reverse_lazy('register_user'), duplicate_register_data, format='json')

        # Assert the response
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY, f"Expected status code 422 but got {response.status_code}"
        assert 'email' in response.data['errors'], f"Expected error for duplicate email but got {response.data['errors']}"
        assert len(response.data['errors']['email']) == 1, f"Expected exactly one error message for email field but got {len(response.data['errors']['email'])}"